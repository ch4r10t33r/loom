...
The implementation is original Zig with two vendored dependencies, both
compiled from source into the static binaries: blockblaz/zig-snappy [13]
(wire compression) and google/brotli v1.1.0 (at-rest chunk text). FAISS,
when present on a node, is loaded at runtime and never required.
