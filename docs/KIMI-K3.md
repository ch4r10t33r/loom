...
Every figure below was read from the published `config.json`, the safetensors
index, and a range request against one weight shard — none of it is estimated.

## Why this model matters to Loom

K3 is the strongest case the architecture has yet had, and also the sharpest
test of its stated limits.

**92.7% of the file is routed experts.** 1.446 TB of the 1.561 TB total is
`block_sparse_moe.experts.*` — immutable, content-addressed, sparsely read. The
non-expert remainder is 114 GB. That ratio is more extreme than GLM 5.2's, and
...
**But the bandwidth math gets worse, not better.** Per token a forward pass
...
closed otherwise — a deliberate safety property, because an mmap hole reads as
zeros and would silently corrupt inference rather than erroring.

114 GB per node is not commodity hardware. Two ways out, and the choice is a
...
- **Requantize the dense part.** It is BF16 today because the MXFP4 recipe
  ...
  This costs quality on precisely the tensors the model authors chose not to
  quantize.
