...
Loom ships three named networks -- Ethereum's chain registry, for LLM
networks. A name resolves to a **stable network id** (a protocol constant:
changing one partitions the network) and the canonical model that network
serves. Every checkpoint here passed `loom gguf check` against this codebase
before earning its row.
...
architecture differs from the network's; `devnet` warns instead -- a PoC
network is where mismatches get discovered on purpose. The one-network-
one-model invariant is what the rest of the system leans on: RAG gossips
text only because every node embeds identically, and expert fetch assumes a
