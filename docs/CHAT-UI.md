...
timings that report decode speed and time to first token separately. On a
partial node the first token also pays for every expert fetch the prefill
needed, so folding the two rates together understates steady-state
throughput badly.
...
calls: no CORS, and no host to configure in the page. Like the API it has
**no TLS and no authentication**, so it binds to loopback by default; do not
widen `--ui-addr` without an authenticating proxy in front.
