...
| `--hold-fraction F` | `1.0` | Fraction of expert shards this node fetches **at bootstrap**, `0.0`–`1.0`. Resident (non-expert) shards are always held in full regardless. **This is not a cap.** A shard fetched from a peer at token time is verified, written to the local store and marked held — fetch-on-demand doubles as heat replication — and nothing evicts it, so a node's holdings only grow. Measured on a two-machine run: a node started at 3.6% of shards and reached 93.1% while generating 24 tokens. If you need a node to *stay* small, this flag will not do it today; there is no eviction policy. |
