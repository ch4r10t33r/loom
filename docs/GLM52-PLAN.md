...
GLM 5.2, a 744B-param MoE (75 MoE layers × 256 experts, 8+1 active), served
by a swarm of commodity machines. A bootnode onboards each joiner and assigns
it the most under-replicated shards first (coverage-aware, from gossiped
holdings). The bootnode is never in the inference path and is not a permanent
dependency.
...
at 50 GB/node, R=2 needs ≥15 nodes and R=3 needs ≥22; at 100 GB/node, 8 nodes
...
Caveats: the engine's kernels are still scalar (SIMD pending). Single
low-latency streams can't fully hide per-layer fetch latency; this is a
serving-throughput architecture, so batch and pin. On 1 GbE the win is
